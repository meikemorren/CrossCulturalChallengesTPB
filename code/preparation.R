...
data[data$Country=="South-Africa","Country"] <- "South Africa"
data[data$Country=="Korea","Country"] <- as.character("South Korea")
data[data$Country=="Rusland","Country"] <- as.character("Russia")
data[data$Country=="The Netherlands","Country"] <- as.character("Netherlands")
data[data$Country=="German","Country"] <- as.character("Germany")
...
data[data$label=="Whitmarsh & ONeill (2010)",2]<-c("Whitmarsh & O'Neill (2010)")
data[data$label=="Mangafi? et al (2017)",2] <- "Mangafić et al (2017)" # check!

### inspect
length(unique(data$ID)) # 224
length(unique(data$label)) # 200
length(unique(data$Country)) # 46
...
WIDEdat[WIDEdat$ID==117,c("PBC_SN","ATT_SN","ATT_PBC")] <- 0.00 # se-sn, att-se, att-sn
...
### inspect
length(unique(WIDEdat$Country)) # 46
sum(WIDEdat$N, na.rm=T) # 114796
nrow(WIDEdat) # 224
length(unique(WIDEdat$label)) # 200 voor 5 TPB variables + PN
...
WIDEdat_TPB[WIDEdat_TPB$ID==117,c("PBC_SN","ATT_SN","ATT_PBC")] <- 0.00 # se-sn, att-se, att-sn 
...
### inspect
unique(WIDEdat_TPB$Country) # 46
sum(WIDEdat_TPB$N, na.rm=T) # 105662
nrow(WIDEdat_TPB) # 215
length(unique(WIDEdat_TPB$label)) # 191 voor 5 TPB variables
